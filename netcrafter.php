...
function meta($name = null)
{
    ...
    global $global_script_meta;
    if (!$global_script_meta) {
        $global_script_meta = meta_from_file(SCRIPT_FILENAME);
    }

    if ($name) return $global_script_meta[$name];
    else return $global_script_meta;
}

/**
* Returns the whole meta data array or a single value from the meta data 
* found within a specified file. Also includes the file path in the 
* returned array.
*
...
function meta_from_files($paths) 
{
    $files_meta = array();

    foreach ($paths as $path) {
        $files_meta[] = meta_from_file($path);
    }
